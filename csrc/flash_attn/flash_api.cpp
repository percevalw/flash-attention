...
void set_params_dgrad(Flash_bwd_params &params,
    ...
    params.dqp_ptr = dqp.data_ptr();
    params.dq_row_stride = dq.stride(-3);
    params.dk_row_stride = dk.stride(-3);
    params.dv_row_stride = dv.stride(-3);
    params.dqp_row_stride = dqp.stride(-3);
    params.dq_head_stride = dq.stride(-2);
    params.dk_head_stride = dk.stride(-2);
    params.dv_head_stride = dv.stride(-2);
    params.dqp_head_stride = dqp.stride(-2);
    ...
    if (cu_seqlens_q_d == nullptr) {
        ...
        params.dqp_batch_stride = dqp.stride(0);
    }

    params.dq_accum_ptr = dq_accum_d;
    params.dk_accum_ptr = dk_accum_d;
    params.dv_accum_ptr = dv_accum_d;
    params.dqp_accum_ptr = dqp_accum_d;
...
mha_fwd(at::Tensor &q,         // batch_size x seqlen_q x num_heads x head_size
    ...
    set_params_fprop(params,
                     ...
                     num_pos,
                     q_padded, k_padded, v_padded, qp_padded, out,
...
mha_bwd(const at::Tensor &dout,  // batch_size x seqlen_q x num_heads, x head_size_og
        ...
        c10::optional<at::Tensor> &dqp_,  // batch_size x seqlen_k x num_heads_k x num_pos
        c10::optional<at::Tensor> &alibi_slopes_, // num_heads or batch_size x num_heads
    ...
    set_params_dgrad(params,
                     ...
                     num_pos,
                     q, k, v, qp, out,
                     dout_padded, dq, dk_expanded, dv_expanded, dqp,
                     nullptr,
                     nullptr,
                     loop ? dq_accum.data_ptr() : nullptr,
                     nullptr, // loop ? dk_accum.data_ptr() : nullptr,
                     nullptr, // loop ? dv_accum.data_ptr() : nullptr,
                     loop ? dqp_accum.data_ptr() : nullptr,
                     softmax_lse.data_ptr(),
                     softmax_d.data_ptr(),
                     p_dropout,
                     softmax_scale,
...
mha_varlen_bwd(const at::Tensor &dout,  // total_q x num_heads, x head_size
               ...
               const at::Tensor &v,   // total_k x num_heads_k x head_size, total_k := \sum_{i=0}^{b} s_i
                const c10::optional<const at::Tensor> &qp,   // total_k x num_heads_k x num_pos, total_k := \sum_{i=0}^{b} s_i
               const at::Tensor &out,   // total_q x num_heads x head_size
               const at::Tensor &softmax_lse,     // b x h x s   softmax logsumexp
               c10::optional<at::Tensor> &dq_,   // total_q x num_heads x head_size, total_q := \sum_{i=0}^{b} s_i
               c10::optional<at::Tensor> &dk_,   // total_k x num_heads_k x head_size, total_k := \sum_{i=0}^{b} s_i
               c10::optional<at::Tensor> &dv_,   // total_k x num_heads_k x head_size, total_k := \sum_{i=0}^{b} s_i
               c10::optional<at::Tensor> &dqp_,  // total_k x num_heads_k x num_pos, total_k := \sum_{i=0}^{b} s_i
               const at::Tensor &cu_seqlens_q,  // b+1
               const at::Tensor &cu_seqlens_k,  // b+1
               c10::optional<at::Tensor> &alibi_slopes_, // num_heads or b x num_heads
